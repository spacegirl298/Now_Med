...
//
// Two tabs cover the patient-facing features: "Intake form" (the digital
// form the patient filled in on first login, with a safe import into the
// verified record) and "Requests" (change requests for details patients
// can't edit themselves; a secretary makes the change in the relevant tab,
// then marks the request done or declines it).
import { useEffect, useMemo, useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { X, Plus, Trash2, ClipboardList, MessageSquare } from 'lucide-react'
...
function tsToDisplay(ts) {
  ...
  return d ? formatDisplayDate(formatDate(d)) : ''
}
...
export default function PatientRecordModal({ patient, appointments = [], initialTab = 'overview', autoOpenAddForm = false, onClose }) {
  ...
  // Intake form: by uid once registered, by ID number for walk-ins.
  useEffect(() => {
    if (!patient) return
    const load = patient.id ? getIntakeForm(patient.id) : getIntakeFormByIdNumber(patient.idNumber)
    load.then((f) => setIntake(f)).catch(() => setIntake(null))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [patientKey])

  // Change requests only exist for registered patients (they file them).
  useEffect(() => {
    if (!patient?.id) return
    const unsub = subscribeToPatientChangeRequests(patient.id, setRequests, (err) => console.error(err))
    return () => unsub && unsub()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [patient?.id])
  ...
  // Copies patient-reported intake answers into the verified profile, filling
  // blanks only. Keeps the overview edit form in sync too - otherwise saving
  // it later would write its stale copy of the profile back over the import.
  async function importIntake() {
    const updates = intakeToProfileUpdates(intake.answers, profile)
    const count = Object.keys(updates).length
    if (count > 0) {
      await persistProfile(updates)
      setOverviewForm((prev) => ({ ...prev, ...updates }))
    }
    await markIntakeImported(intake.id, currentUser?.uid)
    setIntake((prev) => ({ ...prev, importedAt: new Date() }))
    return count
  }
  ...
  const tabs = [
    ...
    ...(patient.id
      ? [{ id: 'requests', label: pendingRequests ? `Requests (${pendingRequests})` : 'Requests' }]
      : []),
  ]
      ...
      <div className="bg-white rounded-2xl w-full max-w-4xl shadow-lg max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-sand shrink-0">
          ...
          <div className="flex items-center gap-4">
            {patient.id && (
              <button
                onClick={() => navigate('/secretary/messages', { state: { patientId: patient.id } })}
                className="flex items-center gap-1.5 text-xs font-medium text-rose hover:underline"
              >
        ...
        <div className="px-6 py-5 overflow-y-auto grow">
          {loadingProfile ? (
            ...
            <>
              ...
              {activeTab === 'intake' && <IntakeTab intake={intake} onImport={importIntake} />}

              {activeTab === 'requests' && patient.id && (
                <RequestsTab requests={requests} currentUser={currentUser} onGoToTab={setActiveTab} />
              )}
...
function IntakeTab({ intake, onImport }) {
  ...
  if (!intake) {
    return (
      <EmptyState
        ...
        message="The patient hasn't completed the digital intake form."
      />
    )
  }
  ...
  async function handleImport() {
    ...
    try {
      ...
      setResult(
        count > 0
          ? `Added ${count} section${count === 1 ? '' : 's'} to the record.`
          : 'Everything from the form is already on the record.',
      )
    } catch (err) {
      console.error(err)
      setError('Could not update the record. Please try again.')
    }
    setImporting(false)
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <SectionCard
        ...
        action={
          intake.importedAt ? (
            <span className="text-xs text-slate">Added to record</span>
          ) : (
            <button
            ...
            >
              {importing ? 'Adding...' : 'Add to patient record'}
            </button>
          )
        }
      >
        <p className="text-xs text-slate">
          Submitted {tsToDisplay(intake.submittedAt) || 'just now'}. This is what the patient told us
          - it is not verified. “Add to patient record” only fills sections that are still empty and
          never overwrites anything already on the record.
        </p>
...
const REQUEST_STATUS = {
  ...
  completed: { label: 'Done', classes: 'bg-sand text-green' },
  declined: { label: 'Declined', classes: 'bg-pastel-red text-red' },
}

// Where to go to actually make the change for each request type.
const SECTION_TAB = {
  'Date of birth': 'overview',
  Gender: 'overview',
  'Name or ID number': 'overview',
  'Blood group': 'overview',
  'Primary doctor': 'overview',
  Allergies: 'allergies',
...
function RequestsTab({ requests, currentUser, onGoToTab }) {
  ...
  async function resolve(request, status) {
    ...
    if (status === 'declined' && !note) {
      setError('Add a short reason so the patient knows why.')
      return
    }
    setBusyId(request.id)
    setError('')
    try {
      await resolveChangeRequest({ request, status, note, resolvedBy: currentUser?.uid || null })
    } catch (err) {
      console.error(err)
  ...
  if (requests.length === 0) {
    return (
      <EmptyState
        ...
        message="If the patient asks for a correction to their record it will show up here."
      />
    )
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {requests.map((r) => {
        ...
        const tab = SECTION_TAB[r.section]
        return (
          <div key={r.id} className="bg-mist rounded-xl p-4">
            <div className="flex items-center justify-between gap-2 mb-1">
              <p className="text-sm font-semibold text-ink">{r.section}</p>
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${status.classes}`}>
                {status.label}
              </span>
            </div>
            <p className="text-xs text-slate mb-2">Requested {tsToDisplay(r.createdAt) || 'just now'}</p>
            {r.currentValue && (
              <p className="text-xs text-slate">
                On record: <span className="text-ink">{r.currentValue}</span>
              </p>
            )}
            <p className="text-sm text-ink mt-1 whitespace-pre-wrap">{r.requestedChange}</p>
            ...
            {r.status === 'pending' ? (
              <div className="mt-3 flex flex-col gap-2">
                {tab && (
                  <button
                    ...
                    className="self-start text-xs font-medium text-rose hover:underline"
                  >
                    Go to this section to make the change
                  </button>
                )}
                <input
                  value={notes[r.id] || ''}
                  onChange={(e) => setNotes((prev) => ({ ...prev, [r.id]: e.target.value }))}
                  placeholder="Note to patient (required if declining)"
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="bg-rose text-white rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-plum transition-colors disabled:opacity-60"
                  >
                    Mark as done
                  </button>
                  <button
                    ...
                    disabled={busyId === r.id}
                    className="border border-stone text-ink rounded-lg px-3 py-1.5 text-xs font-medium hover:border-rose transition-colors disabled:opacity-60"
                  >
                    Decline
                  </button>
                </div>
              ...
              r.resolutionNote && <p className="text-xs text-slate mt-2">Note: {r.resolutionNote}</p>
            )}
          </div>
        )
      })}
