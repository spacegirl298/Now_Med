...
import { updateUserProfile, getDoctorProfile, saveDoctorProfile } from '../../firebase/firestore'
...
export default function SecretaryProfile() {
  ...
  useEffect(() => {
    setName(currentUser?.displayName || '') // eslint-disable-line react-hooks/set-state-in-effect -- syncing local form state from the auth user
  ...
  return (
    <SecretaryLayout>
      <div className="p-6 md:p-8 max-w-2xl mx-auto flex flex-col gap-6">
        ...
        <button onClick={logout} className="text-sm text-red font-medium self-start">
          Log out
        </button>
      </div>
    </SecretaryLayout>
